...
#define MIN_OK 0
#define MIN_INVALID_ARGUMENT 1
#define MIN_EMPTY_INPUT 2
#define MIN_CONTRACT_MISMATCH 3
...
int32_t bloom_primitive_run_vbuf(const uint8_t *input, size_t input_size,
  ...
  const uint16_t key_id = (uint16_t)((anchor >> 16) & UINT64_C(0xffff));
  ...
  if (count == 0) {
    return MIN_EMPTY_INPUT;
  }

  const uint8_t *payload = input + payload_start;
  uint32_t minimum = read_u32_le(payload);
  ...
  /* Canonical known-size v0.6 stream: BaseShift=3, one scalar u32 block. */
  uint8_t output[36] = {0};
  output[0] = 'V';
  output[1] = 'B';
  output[2] = 'U';
  ...
  write_u64_le(output + 16, 12);
  const uint64_t output_anchor = ((uint64_t)key_id << 16) |
                                 (UINT64_C(32) << 32) | (UINT64_C(1) << 48);
  write_u64_le(output + 24, output_anchor);
  write_u32_le(output + 32, minimum);
